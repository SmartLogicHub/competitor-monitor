...
class PriceTrendServiceTest(unittest.TestCase):
    ...
    def test_returns_none_when_week_prices_are_incomplete_or_too_complex(self):
        self.assertIsNone(classify_price_trend([100, 99, None, 98, 97]))
        self.assertIsNone(classify_price_trend([100, 98, 101, 99, 102]))
    ...
    def test_updates_price_status_cells_without_overwriting_existing_values(self):
        ...
        ws.cell(row=7, column=2).value = "缺失款"
        ...
        self.assertEqual(
            stats,
            PriceTrendWriteStats(total=4, written=2, skipped_existing=1, skipped_incomplete=1, skipped_unclassified=0),
        )
        ...
        self.assertIsNone(ws.cell(row=7, column=8).value)
